...
// line 52
// removed ESP32 reset when switching from wifi to CAN SD mode
// line 166 removed closing SD card and switching to SoftAP mode
...
void WiFiManager::setup()
{
    ...
    if (settings.wifiMode == 2) // BE an AP
    {
        ...
        WiFiEventId_t eventID = WiFi.onEvent([](WiFiEvent_t event, WiFiEventInfo_t info)
                                             {
                                                 Serial.println("SavvyCAN disconnected from WiFi");
                                                 
                                                 // alredy in SoftAP mode, stay there
                                                 // SD or Wifi mode selection is at start up
    ...
    if (settings.wifiMode > 0)
    {
        if (!SysSettings.isWifiConnected)
        {
            ...
            if (needServerInit)
            {
                ...
                ArduinoOTA.setHostname("ESP32_RET_SD");
                    ...
                    for (i = 0; i < MAX_CLIENTS; i++)
                    {
                        if (!SysSettings.clientNodes[i] || !SysSettings.clientNodes[i].connected())
                        {
                            ...
                            else
                            {
                                ...
                                // removed for v1.1 SD or WIFI only 2 options
                                /*
                                if (SD_card_present)
                                {
