    ...
    case BUILD_CAN_FRAME:
        ...
        default:
            ...
            else
            {
                ...
                if (out_bus == 1)
                    canManager.sendFrame(&CAN1, build_out_frame);
            }
            break;
        }
        step++;
...
void GVRET_Comm_Handler::sendFrameToBuffer(CAN_FRAME &frame, int whichBus)
{
    ...
    else
    {
        ...
        // SavvyCAN CSV format example
        // Time Stamp,ID,Extended,Dir,Bus,LEN,D1,D2,D3,D4,D5,D6,D7,D8
        // 8740939,000666,false,Rx,0,8,00,03,00,00,00,00,00,00,

        // micros() is a 32 bit value of 64 bit esp_timer_get_time()
        // 32 bit micros = 71 minutes, 64 bit esp_timer_get_time() = 584,000+ years before roll over!

        // writtenBytes = sprintf((char *)&transmitBuffer[transmitBufferLength], "%llu,%X,", esp_timer_get_time(), frame.id);
        // testing of getting timestamp as soon as possible in esp32_can_builtin.cpp line 398
        writtenBytes = sprintf((char *)&transmitBuffer[transmitBufferLength], "%lu,%X,", frame.timestamp, frame.id);
        ...
        writtenBytes = sprintf((char *)&transmitBuffer[transmitBufferLength], "Rx,%i,%i,", whichBus, frame.length);
        transmitBufferLength += writtenBytes;
        for (int c = 0; c < frame.length; c++)
        {
        ...
        // no need for CR and LF, just LF for SavvyCAN to import data
        sprintf((char *)&transmitBuffer[transmitBufferLength], "\n");
